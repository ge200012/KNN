...
void showResult(const string& algorithm, const path& p1, const path& p2) {
    ...
    printf("algorithm:%-20sstart:%-5dend:%-5dscore:%-10.4lfAR:%-10.4lfMR:%-10.4lfRR:%-10.4lfcost:%7ld\n",
           algorithm.c_str(),
           result.first.first,
           result.first.second,
           result.second,
...
void findMostSimilar(const vector<path>& paths, int query, const string& algorithm, const string& targetFile, int limit) {
    ...
    ofs << "algorithm:" << algorithm << " queryLength:" << paths[query].size() <<  " datasize:" <<
          paths.size() << " score:" << efficientAlgorithm(algorithm, paths[query], results[0]).second << " cost:" << time_end - time_start;
