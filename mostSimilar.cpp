...
double execute_data_to_data(const string & algorithm, const path& p1, const path& p2)
{
    ...
    if (algorithm == "efficientAlgorithm") {
        result = efficientAlgorithm_data_to_data(algorithm, p1, p2);
    }
    else if (algorithm == "exactS") {
        result = exactS_data_to_data(p1, p2);
    } 
    //else if (algorithm == "pss") {
    //    result = pss_data_to_data(p1, p2);
    //} else if (algorithm == "pos") {
    //    result = pos_data_to_data(p1, p2);
    //}
    return result;
}
...
subResult execute(const string & algorithm, const path& p1, const path& p2){
    subResult result;
    if (algorithm == "efficientAlgorithm") {
        result = efficientAlgorithm(algorithm, p1, p2);
    } else if (algorithm == "exactS") {
        result = exactS(p1, p2);
    } else if (algorithm == "pss") {
        result = pss(p1, p2);
...
double lowBoundEstimate(vector<rtree_point> points, const path& p, int skip) {
...
map<int, bool> keyPointFilter(map<int, vector<rtree_point>> points, const path& p, const vector<path>& paths, map<int, double> &lowerBounds, int skip, map<int, bool>& multiLowBounds) {
...
map<int, vector<int>> initGrid(const map<int, vector<rtree_point>>& points) {
    ...
    for (const auto &item : points) {
        for (int i = 0; i < keyNum; ++i) {
            pointGrid[gps2gridid(item.second[i].get<0>(), item.second[i].get<1>())].push_back(item.first);
        }
    }
    return pointGrid;
}
...
map<int, bool> gridBasePruning(map<int, vector<int>> pointGrid, const path& p, const vector<int>& querys, int skip, map<int, bool> multiLowBounds) {
    map<int, int> count;
    for (int i = 0; i < p.size(); i += skip) {
        auto gridid = gps2gridid(p[i].get<0>(), p[i].get<1>());
        for (int j = -1; j < 2; ++j) {
            for (int k = -1; k < 2; ++k) {
...
map<int, map<int, pair<vector<double>,subResult>>> multiSimilar(const vector<path>& paths, vector<int> querys, const string &algorithm, int limit) {
    ...
    map<int, vector<rtree_point>> points;
